...
function Header() {
    ...
    return (
        <>
            <div className='main flex flex-wrap bg-orange-500 w-screen h-auto min-h-12 text-amber-50 place-content-around fixed'>
                <Link to="/">
                ...
                <button className={`button  ${currentVal=="Accessories"?"bg-teal-500":""}`} value="Accessories" onClick={handleClick}>Accessories</button>
                </div>
                <div className='cart h-7 w-7 flex  gap-2 mt-2 text-xl font-medium'>
                    <Link to="/cart" className='flex '>
                        <h2>Cart</h2>
                        <img className=" text-xl" src="/cart.svg" alt="" />
                    </Link>
